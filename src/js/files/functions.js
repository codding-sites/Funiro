...
export let bodyLockStatusCart = true;

export let bodyLockToggleCart = (delay = 0) => {
	if (document.documentElement.classList.contains('lock-cart')) {
		bodyUnlockCart(delay);
	} else {
		bodyLockCart(delay);
	}
}
export let bodyUnlockCart = (delay = 0) => {
	let body = document.querySelector("body");
...
export let bodyLockCart = (delay = 0) => {
	let body = document.querySelector("body");
	if (bodyLockStatusCart) {
		let lock_padding = document.querySelectorAll("[data-lp]");
		for (let index = 0; index < lock_padding.length; index++) {
